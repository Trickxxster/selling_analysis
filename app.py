...
# Демо-данные с десятичной точкой (исправлены запятые на точки)
...
def parse_number_str(x: str) -> float:
    """
    Преобразует строку вида '2 786 131,43' или '2 786 131.43' во float.
    Удаляет пробелы, заменяет запятую на точку (если есть).
    """
    if isinstance(x, (int, float)):
        return float(x)
    # Удаляем пробелы и заменяем запятую на точку (для совместимости с обоими форматами)
...
def load_data(file_or_buffer) -> pd.DataFrame:
    ...
    df.columns = [
        'Артикул', 'Номенклатура', 'Характеристика', 'Количество',
        'Выручка', 'Себестоимость', 'Валовая_прибыль', 'Рентабельность'
    ]
...
selected_products = st.sidebar.multiselect(
    "Товар", options=all_products, default=all_products
)
...
with tab1:
    ...
    sort_option = st.radio(
        "Сортировать по:", ["Выручка", "Валовая_прибыль"], horizontal=True
    )
    ...
    fig_pareto.add_bar(
        x=pareto_df['Номенклатура'], y=pareto_df['Выручка'],
        name='Выручка', marker_color='steelblue'
    )
    fig_pareto.add_scatter(
        x=pareto_df['Номенклатура'], y=pareto_df['Накопленная_доля'] * 100,
...
with tab2:
    ...
    with col_b:
        fig_box = px.box(
            filtered_df, y='Рентабельность', color='Категория',
...
with tab3:
    ...
    with col1:
        ...
        fig_pie = px.pie(
            cat_revenue, values='Выручка', names='Категория',
    ...
    fig_price = px.bar(
        cat_prices, x='Категория',
        y=['Средняя_цена', 'Средняя_себестоимость', 'Средняя_прибыль_на_шт'],
        barmode='group',
        title='Средние показатели на единицу товара по категориям'
    )
...
with tab4:
    ...
    money_cols = ['Выручка', 'Себестоимость', 'Валовая_прибыль',
                  'Цена_за_шт', 'Себестоимость_за_шт', 'Прибыль_за_шт']
